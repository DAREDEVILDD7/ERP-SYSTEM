...
import { ArrowLeft, Edit2, Download, CheckCircle, XCircle, Send, User } from 'lucide-react';
...
export default function QuotationDetail({ quotationId, onBack, onEdit, canApprove, onRefresh }) {
    ...
    try {
      ...
      if (data?.approved_by) {
        const { data: u } = await supabase
          .from('users').select('name, role').eq('user_id', data.approved_by).single();
        setApprover(u);
      }
  ...
  useEffect(() => { load(); }, [quotationId]); // eslint-disable-line
      ...
      {showReject && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 space-y-4">
            ...
            <textarea className="input resize-y" rows={3} value={rejectNote}
              onChange={e => setRejectNote(e.target.value)} placeholder="Optional: explain why this quotation is being rejected…"/>
            <div className="flex justify-end gap-3">
              <button onClick={() => setShowReject(false)} className="btn-secondary">Cancel</button>
              <button onClick={() => handleAction('Rejected', { rejection_reason: rejectNote })}
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="card p-5">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                {q.customers?.industry && <p className="text-xs text-gray-400">{q.customers.industry}</p>}
              </div>
              <div>
                <p className="text-xs text-gray-400 mb-1">Contact</p>
                <p className="font-medium text-gray-800">{q.customers?.contact_person ?? '—'}</p>
                {q.customers?.phone && <p className="text-xs text-gray-400">{q.customers.phone}</p>}
              </div>
              <div>
                <p className="text-xs text-gray-400 mb-1">Prepared By</p>
                <p className="font-medium text-gray-800">{q.users?.name ?? '—'}</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 mb-1 flex items-center gap-1">
                  <User size={11}/> Approved By
                </p>
                <p className={`font-medium ${approver ? 'text-green-700' : 'text-gray-300'}`}>
                  {approver ? approver.name : q.status === 'Rejected' ? 'Rejected' : 'Pending'}
                </p>
                {approver && <p className="text-xs text-gray-400">{approver.role}</p>}
              </div>
              <div>
                <p className="text-xs text-gray-400 mb-1">Date</p>
                <p className="font-medium text-gray-800">{format(new Date(q.quotation_date),'dd MMM yyyy')}</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 mb-1">Valid Until</p>
                <p className="font-medium text-gray-800">{q.valid_until ? format(new Date(q.valid_until),'dd MMM yyyy') : '—'}</p>
              </div>
              {q.requirement_id && (
                <div className="col-span-2">
                  <p className="text-xs text-gray-400 mb-1">Requirement</p>
                  <p className="font-medium text-gray-700 font-mono text-xs">{q.requirement_id}</p>
                </div>
              )}
              {q.rejection_reason && (
                <div className="col-span-2">
                  <p className="text-xs text-red-400 mb-1">Rejection Reason</p>
                  <p className="text-sm text-red-700 bg-red-50 rounded-lg p-2">{q.rejection_reason}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="card overflow-hidden">
                  ...
                  {q.quotation_items?.map((item, i) => (
                    <tr key={item.item_id} className="hover:bg-gray-50">
                      ...
                      <td className="px-5 py-3 text-right text-red-500 text-xs">
                        {Number(item.discount_amount??0) > 0 ? `-${fmt(item.discount_amount)}` : '—'}
                      </td>
                      <td className="px-5 py-3 text-right font-medium text-gray-800">
                        {fmt(Math.max(0,(item.total_kwd??item.quantity*item.unit_rate_kwd) - Number(item.discount_amount??0)))}
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="px-5 py-4 border-t border-gray-100 flex flex-col items-end gap-1 text-sm">
              ...
              {Number(q.discount_amount??0) > 0 && <div className="flex gap-12 text-red-500"><span>Discount</span><span className="font-medium">-KWD {fmt(q.discount_amount)}</span></div>}
              ...
              <div className="flex gap-12 font-bold text-gray-900 text-base border-t border-gray-100 pt-2">
                <span>Total</span><span>KWD {fmt(q.total_amount_kwd)}</span>
              </div>
            </div>
          </div>
        </div>
