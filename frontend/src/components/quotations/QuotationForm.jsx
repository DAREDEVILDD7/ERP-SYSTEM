...
const EMPTY_ITEM = {
  ...
  item_type: 'equipment', procurement_id: null,
...
export default function QuotationForm({ existing, prefilledRequirement, onSuccess, onCancel }) {
  ...
  const loadData = useCallback(async () => {
    setDataLoading(true);
    try {
      const [c, e, sm, p, reqs] = await Promise.all([
        getCustomers(),
      ...
      setEquipment(e);
      setStockMap(sm);
      setProcurements(p.filter(x => ['Approved','Delivered'].includes(x.status)));
  ...
  const setItem = (idx, field, val) => {
    setItems(items => items.map((item, i) => {
      ...
      if (field === 'item_type') {
        updated.equipment_id = null; updated.procurement_id = null;
        updated.description = ''; updated.unit_rate_kwd = '';
        updated.rental_start_date = ''; updated.rental_end_date = '';
      }
      return updated;
    }));
  };
  ...
  // Totals
  const itemsSubtotal  = items.reduce((s, i) => s + (Number(i.quantity) * Number(i.unit_rate_kwd || 0)), 0);
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      {/* Mobile: requirement panel shown at top as collapsible */}
      {form.requirement_id && (
      ...
      <form onSubmit={handleSubmit}>
        <div className="flex gap-6">
            ...
            <div className="card p-5 space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <div className="relative flex-1" ref={customerSearchRef}>
                    <div
                      className={clsx('input flex items-center gap-2 cursor-pointer', !form.customer_id && 'text-gray-400')}
                    ...
                    >
                      ...
                      {form.customer_id && (
                        <button type="button" onClick={e => { e.stopPropagation(); set('customer_id', ''); }}
                          className="text-gray-300 hover:text-gray-500 ml-auto shrink-0"><X size={14}/></button>
                      )}
                    </div>
                  ...
                  <button type="button" onClick={() => setShowNewCustomer(true)}
                    className="btn-secondary px-3 text-xs whitespace-nowrap flex items-center gap-1 shrink-0" title="Add new customer">
                    <Plus size={12}/> New
                  </button>
                </div>
              </div>
              ...
              {showNewCustomer && (
                <div className="border-2 border-primary-100 rounded-xl p-4 bg-primary-50/30 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <button type="button" onClick={() => setShowNewCustomer(false)} className="text-gray-400 hover:text-gray-600"><X size={16}/></button>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label className="block text-xs font-medium text-gray-600 mb-1">Company Name *</label>
                      <input className="input text-sm" value={newCustomerForm.company_name}
                        onChange={e => setNewCustomerForm(f => ({...f, company_name: e.target.value}))} required/>
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-600 mb-1">Contact Person *</label>
                      <input className="input text-sm" value={newCustomerForm.contact_person}
                        onChange={e => setNewCustomerForm(f => ({...f, contact_person: e.target.value}))} required/>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <input className="input text-sm" value={newCustomerForm.phone}
                        onChange={e => setNewCustomerForm(f => ({...f, phone: e.target.value}))} placeholder="+965 XXXXXXXX"/>
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-600 mb-1">Email</label>
                      <input type="email" className="input text-sm" value={newCustomerForm.email}
                        onChange={e => setNewCustomerForm(f => ({...f, email: e.target.value}))}/>
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-600 mb-1">Address</label>
                      <input className="input text-sm" value={newCustomerForm.address}
                ...
                <select className="input" value={form.requirement_id} onChange={e => set('requirement_id', e.target.value)}>
                  ...
                  {requirements.map(r => (
                    <option key={r.requirement_id} value={r.requirement_id}>
                      {r.requirement_id} — {r.customers?.company_name} — {r.requirement_summary?.slice(0, 40)}
                    </option>
                  ))}
                </select>
                {reqLoading && (
                  <p className="text-xs text-primary-500 mt-1 flex items-center gap-1">
                    <Loader2 size={11} className="animate-spin"/> Loading requirement details…
                  </p>
                )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="date" className="input" value={form.quotation_date}
                    onChange={e => set('quotation_date', e.target.value)} required/>
                </div>
                <div>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Discount Amount (KWD)</label>
            ...
            <div className="card p-5">
                ...
                {items.map((item, idx) => {
                  ...
                  return (
                    <div key={idx} className={clsx(
                      ...
                      isOverStock ? 'border-red-200 bg-red-50/50' : 'border-gray-100 bg-gray-50/30'
                    )}>
                      {/* Item type tabs */}
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <button type="button" onClick={() => removeItem(idx)} className="ml-auto text-gray-300 hover:text-red-500">
                          <Trash2 size={15}/>
                        </button>
                      </div>
                        ...
                        <div className="space-y-2">
                          <div className="relative">
                            <button type="button"
                              ...
                              className={clsx('input w-full text-left flex items-center justify-between text-sm', isOverStock && 'border-red-300')}>
                              <span className={item.equipment_id ? 'text-gray-800' : 'text-gray-400'}>
                                {item.equipment_id
                                  ? (() => { const eq = equipment.find(e => e.equipment_id === item.equipment_id); return eq ? `${eq.equipment_types?.name} ${eq.capacity} — ${eq.equipment_id}` : item.equipment_id; })()
                                  : 'Search and select equipment…'}
                              </span>
                              {isEqOpen ? <ChevronUp size={14} className="text-gray-400 shrink-0"/> : <ChevronDown size={14} className="text-gray-400 shrink-0"/>}
                            </button>
                          ...
                          {stockInfo && (
                            <div className={clsx('flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium',
                              ...
                              <div className="ml-1 flex-1 max-w-16 h-1.5 bg-white/60 rounded-full overflow-hidden">
                                <div className={clsx('h-full rounded-full', stockInfo.ok ? 'bg-green-500' : 'bg-red-500')}
                                  style={{ width: `${Math.min(100, (stockInfo.available / Math.max(stockInfo.usedInForm, stockInfo.available, 1)) * 100)}%` }}/>
                              </div>
                              {!stockInfo.ok && <span className="font-bold">EXCEEDS STOCK</span>}
                            </div>
                          )}
                        </div>
                      )}
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                        <div>
                          <label className="block text-xs text-gray-400 mb-1">Rental Start</label>
                        ...
                        <div>
                          <label className="block text-xs text-gray-400 mb-1">Rental End / Return</label>
                          <input type="date" className="input text-xs" value={item.rental_end_date}
                            onChange={e => setItem(idx, 'rental_end_date', e.target.value)}/>
                        </div>
                        <div>
                          <label className="block text-xs text-gray-400 mb-1">Item Discount (KWD)</label>
                          <input type="number" min="0" step="0.001" className="input text-xs" placeholder="0.000"
                            value={item.discount_amount} onChange={e => setItem(idx, 'discount_amount', e.target.value)}/>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-gray-400">
                          {item.rental_start_date && item.rental_end_date ? `${item.rental_start_date} → ${item.rental_end_date}` : ''}
                        </span>
                        <span className="font-semibold text-gray-700">
                          Line: KWD {Math.max(0, (Number(item.quantity) * Number(item.unit_rate_kwd || 0)) - Number(item.discount_amount || 0)).toLocaleString('en-US', { minimumFractionDigits: 3 })}
                        </span>
                      </div>
                    </div>
                  );
              ...
              <div className="mt-6 border-t border-gray-100 pt-4 space-y-1.5 flex flex-col items-end text-sm">
                <div className="flex gap-10 text-gray-600">
                  <span>Subtotal</span>
                  <span className="font-medium w-44 text-right">KWD {itemsSubtotal.toLocaleString('en-US', { minimumFractionDigits: 3 })}</span>
                </div>
                {headerDiscount > 0 && (
                  <div className="flex gap-10 text-red-500">
                    <span>Discount</span>
                    <span className="font-medium w-44 text-right">- KWD {headerDiscount.toLocaleString('en-US', { minimumFractionDigits: 3 })}</span>
                  </div>
                )}
                {headerDiscount > 0 && (
                  <div className="flex gap-10 text-gray-600">
                    <span>After Discount</span>
                    <span className="font-medium w-44 text-right">KWD {afterDiscount.toLocaleString('en-US', { minimumFractionDigits: 3 })}</span>
                  </div>
                )}
                {Number(form.vat_percent) > 0 && (
                  <div className="flex gap-10 text-gray-600">
                    <span>VAT ({form.vat_percent}%)</span>
                    <span className="font-medium w-44 text-right">KWD {vatAmt.toLocaleString('en-US', { minimumFractionDigits: 3 })}</span>
                  </div>
                )}
                <div className="flex gap-10 font-bold text-gray-900 text-base border-t border-gray-200 pt-2 mt-1">
                  <span>Total</span>
                  <span className="w-44 text-right">KWD {total.toLocaleString('en-US', { minimumFractionDigits: 3 })}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-end gap-3 pb-6">
              ...
              <button type="submit" disabled={saving || stockViolations.length > 0}
                className={clsx('btn-primary flex items-center gap-2',
                  (saving || stockViolations.length > 0) && 'opacity-60 cursor-not-allowed')}>
          ...
          {/* Desktop: Requirement side panel — sticky */}
          {form.requirement_id && (
