...
export default function DispatchManagePage() {
  ...
  const load = useCallback(async () => {
    // Don't show full loading spinner — use skeleton instead
    try {
  ...
  useEffect(() => {
    const ch = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "dispatch_items" },
        load,
      )
      .subscribe();
  ...
  const handleQuoteSelect = (qId) => {
    ...
    if (q?.quotation_items) {
      const availFromQuote = q.quotation_items
        .filter(
          (i) => i.equipment_id && i.equipment_units?.status === "Available",
        )
        .map((i) => i.equipment_id);
      setSelectedEqIds(availFromQuote);
      const starts = q.quotation_items
        .filter((i) => i.rental_start_date)
        .map((i) => i.rental_start_date);
      const ends = q.quotation_items
        .filter((i) => i.rental_end_date)
        .map((i) => i.rental_end_date);
      if (starts.length > 0)
        setForm((f) => ({ ...f, dispatch_date: starts[0] }));
      if (ends.length > 0)
        setForm((f) => ({ ...f, return_date: ends[ends.length - 1] }));
    }
  };
  ...
  const handleSave = async (e) => {
    ...
    if (selectedEqIds.length === 0)
      return toast.error("Select at least one equipment");
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">
              Link to an approved quotation and select equipment
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSave} className="space-y-4">
                ...
                {filteredQuotes.length === 0 ? (
                  ...
                  filteredQuotes.map((q) => (
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        handleQuoteSelect(q.quotation_id);
                        setQuoteSearch("");
                      }}
                      className={clsx(
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-gray-400">
                            {q.quotation_id} ·{" "}
                            {q.requirements?.requirement_summary?.slice(0, 50)}
                          </p>
                          {q.approver && (
                            <p className="text-xs text-green-600">
                              Approved by: {q.approver.name}
                            </p>
                          )}
                        </div>
                ...
                <div className="flex items-start justify-between">
                  <div>
                    <p className="text-sm font-semibold text-green-800">
                      {selectedQuote.quotation_id} —{" "}
                      {selectedQuote.customers?.company_name}
                      {selectedQuote.customers?.industry &&
                        ` (${selectedQuote.customers.industry})`}
                    </p>
                    <p className="text-xs text-green-600 mt-0.5">
                      {selectedQuote.requirements?.requirement_summary}
                    </p>
                    {selectedQuote.approver && (
                      <p className="text-xs text-green-500 mt-0.5 flex items-center gap-1">
                        <User size={11} /> Approved by:{" "}
                        {selectedQuote.approver.name}
                      </p>
                    )}
                  </div>
                  <button
                ...
                {quoteEquipment.length > 0 && (
                  <div className="mt-3 space-y-1">
                    <p className="text-xs font-medium text-green-700 mb-2">
                      Equipment in quotation:
                    </p>
                    {quoteEquipment.map((item) => {
                      const isAvail =
                        item.equipment_units?.status === "Available";
                      const isSelected = selectedEqIds.includes(
                        item.equipment_id,
                      );
                      return (
                        <div
                        ...
                        >
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-medium text-gray-700">
                              {item.equipment_units?.equipment_types?.name}{" "}
                              {item.equipment_units?.capacity}
                            </span>
            ...
            {selectedEqIds.length > 0 && (
              <div className="bg-primary-50 rounded-xl p-3 space-y-1">
                ...
                {selectedEqIds.map((id) => {
                  const eq =
                    allEquipment.find((e) => e.equipment_id === id) ||
                    quoteEquipment.find((i) => i.equipment_id === id)
                      ?.equipment_units;
                  return (
                    <div
                      key={id}
                    ...
                    >
                      <span className="text-primary-800">
                        {eq?.equipment_types?.name ?? "Unknown"} {eq?.capacity}{" "}
                        — {id}
                      </span>
                      <button
                        type="button"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, dispatch_date: e.target.value }))
                  }
                />
              </div>
              <div>
          ...
          <div className="flex justify-end gap-3 pb-6">
            ...
            >
              ...
              {formLoading
                ? "Creating…"
                : `Create Dispatch (${selectedEqIds.length} item${selectedEqIds.length !== 1 ? "s" : ""})`}
            </button>
          </div>
        </form>
      </div>
                ...
                {filteredDispatches.map((d) => (
                  <>
                    <tr
                      ...
                      onClick={() =>
                        setExpandedId(
                          expandedId === d.dispatch_id ? null : d.dispatch_id,
                        )
                      }
                    >
                        ...
                        {d.quotationApprover ? (
                          <div>
                            <p className="text-xs font-medium text-green-700">
                              {d.quotationApprover.name}
                            </p>
                            <p className="text-xs text-gray-400">
                              {d.quotationApprover.role}
                            </p>
                          </div>
                        ) : (
                      ...
                      <td className="px-5 py-3 text-gray-400 text-xs">
                        {d.return_date
                          ? format(new Date(d.return_date), "dd MMM yyyy")
                          : "—"}
                      </td>
                      <td className="px-5 py-3">
                            ...
                            {d.dispatch_items?.map((item) => (
                              ...
                              >
                                ...
                                <div className="flex-1 min-w-0">
                                  <p className="text-sm font-medium text-gray-800">
                                    {
                                      item.equipment_units?.equipment_types
                                        ?.name
                                    }{" "}
                                    {item.equipment_units?.capacity}
                                  </p>
                                  <p className="text-xs text-gray-400">
                                    {item.equipment_id} · Serial:{" "}
                                    {item.equipment_units?.serial_number ?? "—"}{" "}
                                    · {item.equipment_units?.location ?? "—"}
                                  </p>
                                </div>
                                <StatusBadge
                                  status={
                                    item.equipment_units?.status ?? "Unknown"
                                  }
                                />
                              </div>
                            ))}
              ...
              <div key={d.dispatch_id} className="card p-4">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    ...
                    {d.quotation_id && (
                      <button
                        onClick={() => setPreviewQuote(d.quotations)}
                ...
                {d.quotationApprover && (
                  <p className="text-xs text-green-600">
                    Approved by: {d.quotationApprover.name}
                  </p>
                )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 space-y-4">
            ...
            <p className="text-sm text-gray-500">
              ...
              {cancelTarget.quotations?.customers?.company_name &&
                ` — ${cancelTarget.quotations.customers.company_name}`}
            </p>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
            ...
            <div className="flex items-center justify-between p-5 border-b border-gray-100 sticky top-0 bg-white z-10">
              <div>
                <h3 className="font-semibold text-gray-900">
                  {previewQuote.quotation_id}
                </h3>
                <p className="text-sm text-gray-400">
                  {previewQuote.customers?.company_name}
                  {previewQuote.customers?.industry &&
                    ` — ${previewQuote.customers.industry}`}
                  {previewQuote.customers?.contact_person &&
                    ` — ${previewQuote.customers.contact_person}`}
                </p>
              </div>
              <button
                    ...
                    >
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-sm font-semibold text-gray-700">
                          KWD{" "}
                          {Number(item.unit_rate_kwd).toLocaleString("en-US", {
              ...
              <div className="border-t border-gray-100 pt-3 flex flex-col items-end gap-1 text-sm">
                <div className="flex gap-8 text-gray-600">
                  ...
                  <span className="font-bold text-gray-900">
                    KWD{" "}
                    {Number(previewQuote.total_amount_kwd).toLocaleString(
                      "en-US",
                      { minimumFractionDigits: 3 },
                    )}
                  </span>
