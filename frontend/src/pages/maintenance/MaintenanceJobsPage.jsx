...
import { getMaintenanceJobs, createMaintenanceJob, updateMaintenanceJob } from '../../api/maintenance';
...
const EMPTY = { equipment_id:'', issue:'', issue_type:'Mechanical', service_date:'', cost_kwd:0, status:'Open', notes:'', assigned_to:'' };
...
export default function MaintenanceJobsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setJobs(j);
      setEquipment(e);
      setEngineers(u.filter(u => u.role === 'Maintenance Engineer' || u.role === 'Operations Manager'));
  ...
  useEffect(() => {
    const ch = supabase.channel('maintenance-page')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'maintenance' }, load)
      .subscribe();
    return () => ch.unsubscribe();
  }, [load]);

  const openAdd  = () => { setForm({ ...EMPTY, service_date: new Date().toISOString().split('T')[0] }); setSelected(null); setShowModal(true); };
  const openEdit = (j) => {
    setForm({ equipment_id: j.equipment_id, issue: j.issue, issue_type: j.issue_type??'Mechanical', service_date: j.service_date??'', cost_kwd: j.cost_kwd??0, status: j.status, notes: j.notes??'', assigned_to: j.assigned_to??'' });
    setSelected(j);
    setShowModal(true);
  };
  ...
  const quickComplete = async (id) => {
    try {
      await updateMaintenanceJob(id, { status: 'Completed', completion_date: new Date().toISOString().split('T')[0] });
      toast.success('Marked as completed — equipment returned to Available');
      load();
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          ...
          {canWrite && <button onClick={openAdd} className="btn-primary flex items-center gap-2"><Plus size={16} /> Log Issue</button>}
        </div>
      </div>
      ...
      {loading ? <LoadingSpinner fullscreen={false} /> : jobs.length === 0 ? <EmptyState message="No maintenance jobs" icon={Wrench} /> : (
        <>
              ...
              <tbody className="divide-y divide-gray-50">
                {jobs.map(j => (
                  <tr key={j.maintenance_id} className="hover:bg-gray-50">
                    ...
                    <td className="px-5 py-3 text-gray-700 max-w-xs"><p className="truncate">{j.issue}</p></td>
                    <td className="px-5 py-3 text-gray-500 text-xs">{j.issue_type ?? '—'}</td>
                    <td className="px-5 py-3 text-gray-400 text-xs">{j.service_date ? format(new Date(j.service_date), 'dd MMM yyyy') : '—'}</td>
                    ...
                    {canWrite && (
                      <td className="px-5 py-3 flex items-center gap-2">
                        {['Open','In Progress'].includes(j.status) && (
                          <button onClick={() => quickComplete(j.maintenance_id)} className="text-xs bg-green-500 text-white px-2 py-1 rounded-lg">✓ Complete</button>
                        )}
          ...
          <div className="md:hidden space-y-3">
            {jobs.map(j => (
              <div key={j.maintenance_id} className="card p-4" onClick={() => canWrite && openEdit(j)}>
                <div className="flex justify-between items-start mb-1">
                  <p className="font-medium text-gray-800">{j.equipment_units?.equipment_types?.name}</p>
                  <StatusBadge status={j.status} />
                </div>
                <p className="text-sm text-gray-600 mt-1">{j.issue}</p>
                <p className="text-xs text-gray-400 mt-1">{j.issue_type} · {j.equipment_units?.equipment_id}</p>
                {canWrite && ['Open','In Progress'].includes(j.status) && (
                  <button onClick={(e) => { e.stopPropagation(); quickComplete(j.maintenance_id); }} className="mt-2 text-xs bg-green-500 text-white px-3 py-1 rounded-lg">Mark Complete</button>
                )}
              </div>
            ))}
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-5 border-b border-gray-100">
              <h3 className="font-semibold text-gray-900">{selected ? 'Edit Job' : 'Log Maintenance Issue'}</h3>
            ...
            <form onSubmit={handleSave} className="p-5 space-y-4">
              ...
              <div>
                ...
                <textarea className="input" rows={3} value={form.issue} onChange={e => setForm(f => ({ ...f, issue: e.target.value }))} placeholder="Describe the issue in detail…" required />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Issue Type</label>
                  <select className="input" value={form.issue_type} onChange={e => setForm(f => ({ ...f, issue_type: e.target.value }))}>
                ...
                <div>
                  ...
                  <input type="date" className="input" value={form.service_date} onChange={e => setForm(f => ({ ...f, service_date: e.target.value }))} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Cost (KWD)</label>
                  <input type="number" min="0" step="0.001" className="input" value={form.cost_kwd} onChange={e => setForm(f => ({ ...f, cost_kwd: e.target.value }))} />
                </div>
              </div>
              <div>
