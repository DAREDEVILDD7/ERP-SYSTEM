...
import {
  ...
  Send, Download, Trash2, CheckCircle
} from 'lucide-react';
...
const EMPTY_VENDOR  = { name:'', contact_person:'', phone:'', email:'', address:'', country:'Kuwait', category:'', payment_terms:'', notes:'' };
...
export default function ProcurementPage() {
  ...
  // Modal control: null | 'proc' | 'po' | 'vendor' | 'vendor-inline'
  ...
  const [procForm, setProcForm] = useState({
    title:'', description:'', type:'Purchase', vendor_id:'', priority:'Normal',
    required_by_date:'', lease_start_date:'', lease_end_date:'',
    lease_monthly_kwd:'', terms_conditions:'Standard procurement terms apply.',
    notes:'', status:'Draft',
  });
  ...
  const handleProcSave = async (e) => {
    ...
    try {
      if (selected) {
        ...
        await createProcurement({ ...procForm, requested_by: profile.user_id }, procItems.map(i => ({ ...i, equipment_type_id: i.equipment_type_id || null })));
  ...
  const handleDelivered = async (po) => {
    try {
      await updatePurchaseOrder(po.po_id, { status: 'Delivered', actual_delivery: new Date().toISOString().split('T')[0] });
  ...
  const handleVendorSave = async (e) => {
    ...
    try {
      ...
      if (showModal === 'vendor-inline') {
        // Set vendor in procurement form and go back
  ...
  const setProcItem = (idx, field, val) => setProcItems(items =>
  ...
  // ── Vendor form modal (shared for 'vendor' and 'vendor-inline') ──
  const VendorModal = () => (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/30 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-5 border-b border-gray-100">
          <h3 className="font-semibold text-gray-900">
            {showModal === 'vendor-inline' ? 'Add New Vendor' : 'Add Vendor'}
          </h3>
          <button onClick={() => setShowModal(showModal === 'vendor-inline' ? 'proc' : null)}>
        ...
        <form onSubmit={handleVendorSave} className="p-5 space-y-4">
          ...
          <div>
            ...
            <textarea className="input" rows={2} value={vendorForm.notes} onChange={e => setVendorForm(f => ({...f, notes: e.target.value}))} />
          </div>
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={() => setShowModal(showModal === 'vendor-inline' ? 'proc' : null)} className="btn-secondary">Cancel</button>
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {canVendor && <button onClick={() => openVendorAdd(false)} className="btn-secondary flex items-center gap-2"><Plus size={15}/> Add Vendor</button>}
        </div>
      </div>
                  ...
                  {procs.map(p => (
                    <div key={p.procurement_id} className="card p-4">
                      ...
                      <div className="flex gap-2 mt-2 flex-wrap">
                        {canWrite && p.status === 'Draft' && (
                          <button onClick={() => handleProcSubmit(p.procurement_id)} className="text-xs bg-blue-500 text-white px-3 py-1 rounded-lg">Submit</button>
                        )}
                        {canApprove && p.status === 'Pending Approval' && (
                          <button onClick={() => handleProcApprove(p.procurement_id, true)} className="text-xs bg-green-500 text-white px-3 py-1 rounded-lg">Approve</button>
                        )}
                        {canPO && p.status === 'Approved' && (
                          <button onClick={() => openPOAdd(p)} className="text-xs bg-purple-500 text-white px-3 py-1 rounded-lg">Create PO</button>
                        )}
                      </div>
                    </div>
                  ))}
                      ...
                      {pos.map(po => (
                        <tr key={po.po_id} className="hover:bg-gray-50">
                          ...
                          <td className="px-5 py-3 font-semibold text-gray-800">
                            {Number(po.total_amount_kwd).toLocaleString('en-US',{minimumFractionDigits:3})}
                          </td>
                          <td className="px-5 py-3 text-xs">
                            {po.submitted_at ? (
                              <span className="text-green-600">{format(new Date(po.submitted_at),'dd MMM HH:mm')}</span>
                            ) : <span className="text-gray-300">Not submitted</span>}
                          </td>
                          <td className="px-5 py-3"><StatusBadge status={po.status} /></td>
                          <td className="px-5 py-3 flex items-center gap-2">
                            <button onClick={() => downloadPurchaseOrderPDF(po, [])} className="text-gray-400 hover:text-gray-700">
                              <Download size={15}/>
                            </button>
                            {po.status === 'Draft' && (
                              <button onClick={() => handleSubmitPO(po)} className="text-xs bg-blue-500 text-white px-2 py-1 rounded-lg flex items-center gap-1">
                                <Send size={11}/> Submit
                              </button>
                            )}
                            {['Submitted','Acknowledged','Partially Delivered'].includes(po.status) && (
                              <button onClick={() => handleDelivered(po)} className="text-xs bg-green-500 text-white px-2 py-1 rounded-lg flex items-center gap-1">
                  ...
                  {pos.map(po => (
                    <div key={po.po_id} className="card p-4">
                      ...
                      <p className="text-xs text-gray-400 mt-0.5">
                        {po.submitted_at ? `Submitted: ${format(new Date(po.submitted_at),'dd MMM')}` : 'Not submitted'}
                      </p>
          ...
          {tab === 'Vendors' && (
            <>
              <div className="flex justify-end">
                {canVendor && (
                  <button onClick={() => openVendorAdd(false)} className="btn-primary flex items-center gap-2">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleProcSave} className="p-5 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Notes</label>
                <textarea className="input" rows={2} value={procForm.notes} placeholder="Any additional notes or requirements…" onChange={e=>setProcForm(f=>({...f,notes:e.target.value}))}/>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button type="button" onClick={addProcItem} className="text-xs text-primary-500 hover:underline flex items-center gap-1">
                    ...
                    <div key={idx} className="border border-gray-100 rounded-xl p-3 space-y-2 bg-gray-50">
                      <div className="grid grid-cols-12 gap-2 items-center">
                        <div className="col-span-12 sm:col-span-4">
                          <select className="input text-xs" value={item.equipment_type_id}
                            onChange={e => setProcItem(idx,'equipment_type_id',e.target.value)}>
                            <option value="">Equipment type (optional)</option>
                            {eqTypes.map(t=><option key={t.type_id} value={t.type_id}>{t.name}</option>)}
                          </select>
                        </div>
                        <div className="col-span-12 sm:col-span-4">
                          <input className="input text-xs" placeholder="Description *" value={item.description}
                            onChange={e=>setProcItem(idx,'description',e.target.value)} required/>
                        </div>
                        <div className="col-span-4 sm:col-span-1">
                          <input type="number" min="1" className="input text-xs" placeholder="Qty" value={item.quantity}
                            onChange={e=>setProcItem(idx,'quantity',e.target.value)}/>
                        </div>
                        <div className="col-span-6 sm:col-span-2">
                          <input type="number" min="0" step="0.001" className="input text-xs" placeholder="Unit Price KWD"
                            value={item.unit_price_kwd} onChange={e=>setProcItem(idx,'unit_price_kwd',e.target.value)}/>
                        </div>
                        <div className="col-span-2 sm:col-span-1 flex justify-end">
                          <button type="button" onClick={()=>removeProcItem(idx)} className="text-red-400 hover:text-red-600">
                            <Trash2 size={14}/>
                          </button>
                        </div>
                      </div>
      ...
      {/* ── Vendor Modals (standalone + inline) ── */}
      {(showModal === 'vendor' || showModal === 'vendor-inline') && <VendorModal />}
    </div>
  );
}