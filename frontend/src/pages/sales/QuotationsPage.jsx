...
export default function QuotationsPage() {
  ...
  const hasActiveFilters =
    status !== "All" || search || preparedBy || dateFrom || dateTo;
  const clearFilters = () =>
    setQuotationsFilters({
      search: "",
      status: "All",
  ...
  const handleDelete = async (e, q) => {
    ...
    if (
      !window.confirm(
        `Delete quotation ${q.quotation_id}? This cannot be undone.`,
      )
    )
      return;
    ...
    try {
      ...
      setQuotations(
        quotations.filter((x) => x.quotation_id !== q.quotation_id),
      );
    } catch (err) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="relative">
        ...
        {search && (
          <button
            onClick={() => setQuotationsFilters({ search: "" })}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {STATUSES.map((s) => (
                <button
                  key={s}
                  onClick={() => setQuotationsFilters({ status: s })}
                  className={clsx(
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div>
              ...
              <input
                ...
                value={preparedBy}
                onChange={(e) =>
                  setQuotationsFilters({ preparedBy: e.target.value })
                }
              />
            </div>
            <div>
              ...
              <input
                ...
                value={dateFrom}
                onChange={(e) =>
                  setQuotationsFilters({ dateFrom: e.target.value })
                }
              />
            </div>
            <div>
              ...
              <input
                ...
                value={dateTo}
                onChange={(e) =>
                  setQuotationsFilters({ dateTo: e.target.value })
                }
              />
            </div>
          </div>
      ...
      {hasActiveFilters && !showFilters && (
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {status !== "All" && (
            <FilterPill
              label={`Status: ${status}`}
              onRemove={() => setQuotationsFilters({ status: "All" })}
            />
          )}
          {preparedBy && (
            <FilterPill
              label={`By: ${preparedBy}`}
              onRemove={() => setQuotationsFilters({ preparedBy: "" })}
            />
          )}
          {dateFrom && (
            <FilterPill
              label={`From: ${dateFrom}`}
              onRemove={() => setQuotationsFilters({ dateFrom: "" })}
            />
          )}
          {dateTo && (
            <FilterPill
              label={`To: ${dateTo}`}
              onRemove={() => setQuotationsFilters({ dateTo: "" })}
            />
          )}
          <button
            onClick={clearFilters}
                  ...
                  {filtered.map((q) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-3 text-right text-sm">
                        {Number(q.discount_amount ?? 0) > 0 ? (
                          <span className="text-red-500">
                            -
                            {Number(q.discount_amount).toLocaleString("en-US", {
                              minimumFractionDigits: 3,
                            })}
                          </span>
                        ) : (
                      ...
                      <td className="px-5 py-3 text-right font-semibold text-gray-800">
                        {Number(q.total_amount_kwd).toLocaleString("en-US", {
                          minimumFractionDigits: 3,
                        })}
                      </td>
                      ...
                      <td className="px-5 py-3 text-xs">
                        {q.requirement_id ? (
                          <button
                            ...
                            className="font-mono text-primary-600 hover:underline flex items-center gap-1"
                          >
