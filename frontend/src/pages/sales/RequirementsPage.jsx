...
import {
  Plus, Search, Filter, RefreshCw, ClipboardList, X,
} from 'lucide-react';
...
export default function RequirementsPage() {
  ...
  // Client-side filtering — includes creator search
  const filtered = requirements.filter(r => {
    ...
    const matchCreator  = !createdBy || r.users?.name?.toLowerCase().includes(createdBy.toLowerCase());

    return matchSearch && matchStatus && matchPriority && matchDateFrom && matchDateTo && matchCustomer && matchCreator;
  });

  const hasActiveFilters = status !== 'All' || priority !== 'All' || search || dateFrom || dateTo || customer || createdBy;

  const clearFilters = () => setRequirementsFilters({
    search:'', status:'All', priority:'All', dateFrom:'', dateTo:'', customer:'', createdBy:'',
  });
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => { clearRequirementsCache(); load(true); }} className="btn-secondary p-2" title="Refresh">
            <RefreshCw size={16}/>
          </button>
          <button
      ...
      <div className="relative">
        ...
        {search && (
          <button onClick={() => setRequirementsFilters({ search: '' })}
        ...
        <div className="card p-4 space-y-4 border-2 border-primary-100">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {STATUSES.map(s => (
                <button key={s} onClick={() => setRequirementsFilters({ status: s })}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {PRIORITIES.map(p => (
                <button key={p} onClick={() => setRequirementsFilters({ priority: p })}
          ...
          {/* Date + Customer + Creator */}
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1">Customer</label>
              <input className="input text-sm" placeholder="Filter by customer…"
                value={customer} onChange={e => setRequirementsFilters({ customer: e.target.value })}/>
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1">Created By</label>
              <input className="input text-sm" placeholder="Filter by creator name…"
                value={createdBy ?? ''} onChange={e => setRequirementsFilters({ createdBy: e.target.value })}/>
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1">From Date</label>
              <input type="date" className="input text-sm" value={dateFrom}
                onChange={e => setRequirementsFilters({ dateFrom: e.target.value })}/>
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1">To Date</label>
              <input type="date" className="input text-sm" value={dateTo}
                onChange={e => setRequirementsFilters({ dateTo: e.target.value })}/>
            </div>
          </div>
        </div>
      )}
      ...
      {hasActiveFilters && !showFilters && (
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {createdBy          && <FilterPill label={`By: ${createdBy}`}      onRemove={() => setRequirementsFilters({ createdBy: '' })}/>}
                ...
                <tbody className="divide-y divide-gray-50">
                  {filtered.map(r => (
                    <tr key={r.requirement_id}
                      ...
                      <td className="px-4 py-3 font-mono text-xs text-gray-400 whitespace-nowrap">{r.requirement_id}</td>
                      ...
                      <td className="px-4 py-3 text-gray-600 whitespace-nowrap text-sm">{r.customers?.company_name}</td>
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-xs text-gray-600">{r.users?.name ?? '—'}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-gray-400 text-xs whitespace-nowrap">{r.location ?? '—'}</td>
                      ...
                      <td className="px-4 py-3 text-gray-400 text-xs whitespace-nowrap">
                        <p>{format(new Date(r.created_at), 'dd MMM yyyy')}</p>
                        <p className="text-gray-300">{format(new Date(r.created_at), 'HH:mm')}</p>
                      </td>
                    </tr>
                  ))}
                </tbody>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    <div className="w-5 h-5 rounded-full bg-primary-100 flex items-center justify-center">
                      <span className="text-primary-600 text-xs font-semibold">{r.users?.name?.charAt(0) ?? 'U'}</span>
                    </div>
                    <span className="text-xs text-gray-500">{r.users?.name ?? '—'}</span>
                  </div>
                  <span className="text-xs text-gray-400">{format(new Date(r.created_at), 'dd MMM yyyy, HH:mm')}</span>
                </div>
